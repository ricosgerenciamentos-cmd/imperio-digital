...
const filters=[
['todos','Todos'],['ganhar','Quero ganhar dinheiro'],['saude','Quero cuidar da saúde'],['estudar','Quero estudar melhor'],['organizar','Quero organizar minha rotina'],['nicho','Nicho específico']
];

function wa(p){return `https://wa.me/${WHATSAPP}?text=${encodeURIComponent(p?`Olá! Tenho interesse no ebook ${p.title}`:'Olá! Quero conhecer os ebooks do Império Digital')}`}
...
function App(){
 ...
 const list=useMemo(()=>products.filter(p=>{
   const okFilter=filter==='todos'?!p.hidden:p.intent===filter;
   const okSearch=(p.title+p.cat+p.intent+p.tag).toLowerCase().includes(search.toLowerCase());
...
function Header({search,setSearch}){return <header className="header">
 ...
 <a className="headBtn" href="#catalogo">Comprar Agora</a>
 </header>}
...
function Hero({best}){return <section id="top" className="hero">
 <div className="copy">
  ...
  <p className="lead">Ebooks práticos e acessíveis para aprender, empreender, organizar sua rotina e cuidar melhor de você.</p>
  ...
  <div className="actions"><a className="primary" href="#best">Comprar Agora</a><a className="secondary" href="#catalogo">Ver catálogo</a></div>
 </div>
 <div className="scene">
...
function BestSellers({best}){return <section id="best" className="best"><p className="red">MAIS VENDIDOS</p><h2>Escolha seu ebook e comece hoje</h2><p className="sub">Os produtos com maior potencial para compra rápida.</p><div className="bestGrid">{best.map(p=><Product p={p} compact key={p.id}/>)}</div></section>}
...
function MobileBar(){return <div className="mobile"><a href={wa()} target="_blank" rel="noreferrer">WhatsApp</a><a href="#best">Comprar</a></div>}
